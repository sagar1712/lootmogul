...
const HeroSection = () => {
  ...
  const [activeApi, setActiveApi] = useState<'weather' | 'news' | 'openai' | null>(null);

  const fetchWeatherData = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await fetch(`/api/weather-info?city=${encodeURIComponent(weatherCity)}`);
      if (!response.ok) {
  ...
  const fetchNewsData = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/news?query=${encodeURIComponent(newsQuery)}`);
      if (!response.ok) {
      ...
      case 'weather':
        ...
        return (
          <div>
            ...
            <p>Min Temperature: {weatherData.main.temp_min.toFixed(1)}°C</p>
            <p>Max Temperature: {weatherData.main.temp_max.toFixed(1)}°C</p>
          ...
          <div>
            ...
            <ul>
              {newsData.articles.slice(0, 5).map((article, index) => (
                <li key={index}>
                  <a href={article.url} target="_blank" rel="noopener noreferrer" className="text-blue-300 hover:underline">
                    {article.title}
                  </a>
                </li>
              ))}
